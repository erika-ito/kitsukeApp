...
{
    // 中間テーブル（着付対象者）への保存
    public static function save($request, $customer_counts, $insert_reservation_id, $match_customer_id)
    {
        for ($i = 1; $i <= $customer_counts; $i++) {
            ...
            $customer_reservation->customer_id = $match_customer_id[$i-1]; // 配列からIDを取り出す
